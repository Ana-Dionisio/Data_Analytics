...
SELECT AVG(t.amount) as media_ventas, c.company_name 
FROM transaction as t
JOIN company as c
ON t.company_id = c.id
...
WHERE c.id IN 
			...
			HAVING AVG(t.amount) >
					...
					SELECT AVG(t.amount) as media_general FROM transaction as t
					)
			)
    ;
...
-- select para filtrar las empresas com más de 400 transacciones
...
-- select para filtrar las empresas com menos de 400 transacciones
SELECT c.company_name, COUNT(t.id) 
FROM company as c
JOIN transaction as t
ON c.id = t.company_id
